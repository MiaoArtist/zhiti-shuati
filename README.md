...
- **套题重命名**：首页套卷卡片名称旁有「✏️」，点后可重命名；重命名只改本地/云端数据里的 `papers[].name`，不影响做题记录（记录按 `id::题号` 关联）。导出数据自动携带新名称，织题导入时来源标签即用新名称。
- **题概览**：试卷页顶栏「☰」下拉菜单里有「题概览」，弹出全部题号方框：绿=做对、红=做错、灰=未做；点任意方框跳到对应题。
