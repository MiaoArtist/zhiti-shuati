...
- 左滑 = 下一题，右滑 = 上一题；在题目卡片任意区域（题干/选项/答案/心得/底部按钮）都有效，上下滚动过程中横滑也可翻页。
- 滑动时卡片会跟随手指并有左右箭头提示；复制/长按选取文字不会误翻页。
- 已移除震动反馈，横向拖动阻尼系数更低、回弹动画更顺滑；页面加了 `overscroll-behavior-y: contain` 抑制整页橡皮筋（iOS 原生滚动惯性仍由系统控制）。
- 手机端进入某套卷后，点顶栏「☰」展开下拉菜单，「背题 / 刷题」切换在菜单里。

## 套卷管理、题概览与手机端手势

- **套题重命名与删除**：首页套卷卡片上有「✏️」重命名、「🗑」删除（删除会连该卷做题记录一起清掉）。重命名只改本地/云端数据里的 `papers[].name`，不影响做题记录；导出数据自动携带新名称，织题导入时来源标签即用新名称。
- **题概览**：电脑端在试卷页顶部直接有「题概览」按钮；手机端从顶栏「☰」下拉菜单进入。弹窗按顺序显示数字方框：绿=做对、红=做错、灰=未做；点任意方框跳到对应题。
