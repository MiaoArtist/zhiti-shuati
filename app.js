...
function attachSwipe() {
  ...
  const card = $('#qcard');
  if (!card) return;
  let sx = 0, sy = 0, st = 0, sScroll = 0;
  const ignore = e => e.target && e.target.closest && e.target.closest('textarea, input, select, button, .opt-btn');
  ...
  card.addEventListener('touchend', e => {
    ...
    } else if (!scrolled && dy < -70 && Math.abs(dy) > Math.abs(dx) * 1.2 && dur < 450) {
      const p = curP(), q = curQ(), r = getRec(p, q);
