...
function startPaper(i) {
  view = { name: 'paper', paperIdx: i, qIdx: 0, order: null, shuffle: false, filters: { status: '', type: '' }, revealedIds: new Set() };
  renderPaper();
}
...
function buildOrder() {
  ...
  const cur = (view.order && view.order[view.qIdx] != null) ? view.order[view.qIdx] : 0;
...
function renderPaper() {
  ...
  $('#btn-mode-back').classList.toggle('active', mode === 'back');
...
// ================= 事件 =================
function bind() {
  if (IS_MOBILE) { const b = $('#btn-export'); if (b) b.style.display = 'none'; }
